(() => {
 ...
 window.birthdayCelebration=()=>new Promise(resolve=>{
  ...
  function burst(){window.albumEffects?.firework();const x=width*(.12+Math.random()*.76),y=height*(.12+Math.random()*.48),count=width<700?30:45;for(let i=0;i<count;i++){const angle=Math.PI*2*i/count,speed=55+Math.random()*(width<700?100:155);particles.push({x,y,vx:Math.cos(angle)*speed,vy:Math.sin(angle)*speed,life:1.3+Math.random()*.5,age:0,color:colors[i%colors.length]})}particles=particles.slice(-180)}
  ...
  function finish(){if(finished)return;finished=true;window.albumEffects?.stopFireworks();clearTimeout(timer);cancelAnimationFrame(frame);window.removeEventListener('resize',resize);overlay.classList.add('birthday-leaving');setTimeout(()=>{overlay.remove();if(previousFocus?.isConnected)previousFocus.focus({preventScroll:true});resolve()},reduced?0:450)}
